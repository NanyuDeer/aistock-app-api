...
import { computeJq, type BreadthCache, type TushareClient } from '../src/modules/fear-greed/calculator';
...
test('margin 指标标记 excluded（不计入综合指数）', async () => {
    const result = await computeJq(makeClient(), makeCache());
...
test('breadth 缓存 upsert 被调用并写入缺失日期', async () => {
    const cache = makeCache();
    await computeJq(makeClient(), cache);
